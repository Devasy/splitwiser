...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const groupsWithBalance = (groupsRes.data.groups || []).map(
          (group: Group) => {
  ...
  const balanceChartData = [
    {
      name: "Owed To You",
      value: summary?.totalOwedToYou || 0,
      color: "#10b981",
    },
  ...
  const CustomPieTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div
          className={`p-3 shadow-lg ${style === THEMES.GLASSMORPHISM ? "bg-white/90 backdrop-blur-sm rounded-lg" : "bg-white border-2 border-black"}`}
        >
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="flex flex-col items-center justify-center text-center">
          <div
            className={`p-4 mb-4 ${style === THEMES.NEOBRUTALISM ? "bg-emerald-100 text-black border-2 border-black rounded-none" : "bg-emerald-100 text-emerald-600 rounded-full"}`}
          >
            <TrendingUp size={32} />
          </div>
          <p className="opacity-70 font-medium">Owed to You</p>
          <h2
            className={`text-3xl font-extrabold ${style === THEMES.NEOBRUTALISM ? "text-black" : "text-emerald-500"}`}
          >
        ...
        <Card className="flex flex-col items-center justify-center text-center">
          <div
            className={`p-4 mb-4 ${style === THEMES.NEOBRUTALISM ? "bg-red-100 text-black border-2 border-black rounded-none" : "bg-red-100 text-red-600 rounded-full"}`}
          >
            <TrendingDown size={32} />
          </div>
          <p className="opacity-70 font-medium">You Owe</p>
          <h2
            className={`text-3xl font-extrabold ${style === THEMES.NEOBRUTALISM ? "text-black" : "text-red-500"}`}
          >
        ...
        <Card className="flex flex-col items-center justify-center text-center">
          <div
            className={`p-4 mb-4 ${style === THEMES.NEOBRUTALISM ? "bg-blue-100 text-black border-2 border-black rounded-none" : "bg-blue-100 text-blue-600 rounded-full"}`}
          >
            <DollarSign size={32} />
          </div>
          <p className="opacity-70 font-medium">Net Balance</p>
          <h2
            className={`text-3xl font-extrabold ${summary && summary.netBalance >= 0 ? (style === THEMES.NEOBRUTALISM ? "text-black" : "text-emerald-500") : style === THEMES.NEOBRUTALISM ? "text-black" : "text-red-500"}`}
          >
              ...
              <BarChart data={balanceChartData}>
                ...
                <Tooltip
                  contentStyle={{
                    ...
                    borderRadius:
                      style === THEMES.GLASSMORPHISM ? "12px" : "0px",
                    border:
                      style === THEMES.NEOBRUTALISM
                        ? "2px solid black"
                        : "none",
                ...
                <Bar
                  dataKey="value"
                  radius={style === THEMES.GLASSMORPHISM ? [8, 8, 0, 0] : 0}
                  barSize={60}
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  >
                    {friendsBalanceData.map((entry, index) => (
                      <Cell
                        ...
                        fill={entry.color}
                        stroke={
                          style === THEMES.NEOBRUTALISM ? "black" : "none"
                        }
                        strokeWidth={1}
                      />
                    ))}
        ...
        <Card title="Quick Actions">
          <div className="space-y-4 mt-4">
              ...
              .map((group) => (
                ...
                >
                  <div className="text-left">
                    ...
                    <p
                      className={`text-sm ${(group.userBalance || 0) >= 0 ? "text-emerald-500" : "text-red-500"}`}
                    >
            ...
            {groups.filter((g) => Math.abs(g.userBalance || 0) > 0.01)
              .length === 0 && (
            ...
            {summary.groupsSummary.map((groupSum: any, index: number) => {
              ...
              return (
                ...
                >
                  ...
                  <p
                    className={`text-xl font-black mt-2 ${isPositive ? "text-emerald-500" : "text-red-500"}`}
                  >
                  ...
                  <div className="w-full bg-gray-200 h-2 mt-2 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, (yourBalance / Math.max(summary.totalOwedToYou + summary.totalYouOwe, 1)) * 100)}%`,
                      }}
                    />
                  </div>
                </button>
