...
export const getSettlements = async (groupId: string) =>
  api.get(`/groups/${groupId}/settlements?status=pending`);
export const createSettlement = async (
  groupId: string,
  data: { payer_id: string; payee_id: string; amount: number },
) => api.post(`/groups/${groupId}/settlements`, data);
export const getOptimizedSettlements = async (groupId: string) =>
  api.post(`/groups/${groupId}/settlements/optimize`);
export const markSettlementPaid = async (
  groupId: string,
  settlementId: string,
) =>
  api.patch(`/groups/${groupId}/settlements/${settlementId}`, {
    status: "completed",
  });

// Users
export const getBalanceSummary = async () =>
  api.get("/users/me/balance-summary");
export const getFriendsBalance = async () =>
  api.get("/users/me/friends-balance");
...
// Splitwise Import
export const getSplitwiseAuthUrl = async () =>
  api.get("/import/splitwise/authorize");
export const handleSplitwiseCallback = async (
  code?: string,
  state?: string,
...
export const getSplitwisePreview = async (accessToken: string) =>
  api.post("/import/splitwise/preview", { access_token: accessToken });
export const startSplitwiseImport = async () =>
  api.post("/import/splitwise/start");
export const getImportStatus = async (importJobId: string) =>
  api.get(`/import/status/${importJobId}`);
export const rollbackImport = async (importJobId: string) =>
  api.post(`/import/rollback/${importJobId}`);
